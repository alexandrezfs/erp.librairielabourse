    ...
    private function findFichesRea()
    {
        ...
        if ($query->rowCount() == 0) {
            ...
            while ($row = $query->fetch()) {
                ...
                $this->result .= '<td>' . $row['date'] . '</td>';
                $this->result .= '<td>' . htmlspecialchars($row['magasin']) . '</td>';
                $this->result .= '<td><a href="http://services.librairielabourse.fr/reports/product?date=' . $row['date'] . '&magasin=' . htmlspecialchars($row['magasin']) . '&rea=REASSORTI&type=LIVRE"><button class="btn">Consulter</button></a></td>';
                $this->result .= '</tr>';
            }
